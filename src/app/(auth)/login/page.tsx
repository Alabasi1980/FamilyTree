...
function LoginError({ kind }: { kind: ErrorKind }) {
  const messages: Record<ErrorKind, { title: string; hint?: string }> = {
    no_account: {
      title: "لا يوجد حساب بهذا البريد أو رقم الهاتف.",
      hint: "تحقق من الكتابة أو أنشئ حساباً جديداً.",
    },
    google_only: {
      title: "هذا الحساب مرتبط بـ Google فقط.",
      hint: "استخدم زر «تسجيل الدخول بـ Google» أعلاه.",
    },
    wrong_password: {
    ...
    unknown: {
      ...
      hint: "تحقق من بياناتك أو حاول الدخول بـ Google.",
    },
  };

  const { title, hint } = messages[kind];
  return (
    <div className="flex gap-2.5 rounded-md bg-destructive/10 border border-destructive/20 px-3 py-2.5 text-sm">
...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const emailOrPhone = form.get("emailOrPhone") as string;
    ...
    startTransition(async () => {
      ...
      // فحص السبب المحدد للخطأ
      const reason = await checkLoginReason(emailOrPhone);
      if (reason === "no_account") setErrorKind("no_account");
      else if (reason === "google_only") setErrorKind("google_only");
      else setErrorKind("wrong_password");
    });
  }
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="relative mb-5">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="bg-background px-3 text-muted-foreground">أو ادخل ببيانات حسابك</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {errorKind && <LoginError kind={errorKind} />}
