...
files = {
    'src/lib/base-path.ts': '''export function normalizeBasePath(value?: string | null) {
  if (!value) return "";
...
''',
    'src/app/manifest.ts': '''import type { MetadataRoute } from "next";
import { withBasePath } from "@/lib/base-path";
...
''',
    'src/components/pwa-register.tsx': '''"use client";
...
''',
    'src/lib/auth.config.ts': '''import type { NextAuthConfig } from "next-auth";
...
''',
    'public/sw.js': '''// Minimal network-first service worker.
