  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      await signIn("credentials", {
        emailOrPhone: (form.get("email") as string) || (form.get("phone") as string),
        password: form.get("password") as string,
        redirect: false,
      });
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input name="fullName" type="text" placeholder="محمد علي" required className="bg-card/60" />
          </div>

          <div className="space-y-1.5">
            <label className="text-sm font-medium text-foreground">
              البريد الإلكتروني <span className="text-muted-foreground text-xs">(أو رقم الهاتف)</span>
            </label>
            <Input name="email" type="email" placeholder="example@email.com" className="bg-card/60" />
          </div>

          <div className="space-y-1.5">
            <label className="text-sm font-medium text-foreground">
              رقم الهاتف <span className="text-muted-foreground text-xs">(أو البريد الإلكتروني)</span>
            </label>
            <Input name="phone" type="tel" placeholder="05xxxxxxxx" className="bg-card/60" dir="ltr" />
          </div>
